...
import { safeStringify } from 'amper-utils/dist/jsonUtils';
import { Stash } from 'amper-utils';
...
export function validateSchema(schemaNode: Types.Schema, path?: string) {
  if (!isObject(schemaNode) || Object.keys(schemaNode).length === 0) {
...
export function validateFields(
  ...
  for (let propName in fields) {
    ...
    let fieldIsObject = fieldValue && isObject(fieldValue);
    ...
    if ((fieldIsObject || fieldIsArray) && !Types.isType(subSchema)) {
      ...
      if (disallowNewFields) {
        return 'path does not exist in schema: ' + keys.concat([propName]).join('/') + ' schema: ' + safeStringify(schema)
          + ' fields: ' + safeStringify(fields);
      }
